...
import (
	...
	"github.com/willie68/go-micro/internal/shared/utils/httputils"
)
...
// Address is the domain model for address operations.
type Address interface {
	Addresses(ctx context.Context) ([]pmodel.Address, error)
	Has(ctx context.Context, id string) bool
	Read(ctx context.Context, id string) (*pmodel.Address, error)
	Create(ctx context.Context, adr pmodel.Address) (string, error)
	Update(ctx context.Context, adr pmodel.Address) error
...
func NewAdrHandler(inj do.Injector) api.Handler {
	return &AdrHandler{
		adrstg: do.MustInvokeAs[Address](inj),
...
func (c *AdrHandler) GetAddresses(response http.ResponseWriter, request *http.Request) {
	...
	l, err := c.adrstg.Addresses(request.Context())
	if err != nil {
...
func (c *AdrHandler) GetAddress(response http.ResponseWriter, request *http.Request) {
	...
	if !c.adrstg.Has(request.Context(), n) {
		httputils.Err(response, request, serror.ErrNotExists)
		return
	}
	adr, err := c.adrstg.Read(request.Context(), n)
	if err != nil {
...
func (c *AdrHandler) PostAddress(response http.ResponseWriter, request *http.Request) {
	...
	postAdrCounter.Inc()
	n, err := c.adrstg.Create(request.Context(), adr)
	if err != nil {
		httputils.Err(response, request, serror.Wrapc(err, http.StatusInternalServerError))
		return
	}
...
func (c *AdrHandler) UpdateAddress(response http.ResponseWriter, request *http.Request) {
	...
	postAdrCounter.Inc()
	err = c.adrstg.Update(request.Context(), adr)
	if err != nil {
		httputils.Err(response, request, serror.Wrapc(err, http.StatusInternalServerError))
		return
	}
...
func (c *AdrHandler) DeleteAddress(response http.ResponseWriter, request *http.Request) {
	...
	err = c.adrstg.Delete(request.Context(), n)
	if err != nil {
		httputils.Err(response, request, serror.Wrapc(err, http.StatusInternalServerError))
		return
	}
